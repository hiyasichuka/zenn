...
そこで、Rust 製で高速に動作し、**複数言語のバージョン管理・環境変数・タスク実行を一元管理できる**次世代のツールマネージャーである **mise** (旧 rtx) に乗り換えたので、その導入手順と魅力を紹介します。
...
これで、ディレクトリを移動した際に自動的に適切なツールバージョンがセットされるようになります。

## 基本的な使い方

### グローバルのデフォルトバージョンを設定する
...
:::message
**コラム: mise use と mise install の使い分け**
...
anyenv から mise へ移行する際は、以下の手順を推奨します。
...
mise が問題なく動作することを確認できたら、anyenv を削除できます。
