...
実際のアプリケーション開発では、多くのフレームワークで標準サポートされているため、まず検討される方式です。
...
- リクエスト時に番号札を見せると、サーバーが奥から荷物を出してくる
...
- **即時ログアウト（無効化）が難しい**
  ...
  → 有効期限を短くしてリフレッシュトークンを併用したり、ブラックリスト運用をするなどの工夫が必要。
...
管理画面のような閉じた世界ならセッション認証が安全で楽ですし、モバイルアプリを含む大規模サービスなら JWT が適しています。
...
| 本質のイメージ   | ホテルの番号札      | パスポート               |
| データの場所     | サーバーにある      | トークン内にある         |
...
>
> **JWT 認証**はクライアント側に情報を持たせる『パスポート』のような方式です。サーバーはステートレスになり、スケーラビリティが高くマイクロサービス等に適していますが、発行済みトークンの即時無効化が難しいため、有効期限の設計が重要になります。
>
...
- [**OWASP Cheat Sheet Series - JSON Web Token for Java**](https://cheatsheetseries.owasp.org/cheatsheets/JSON_Web_Token_for_Java_Cheat_Sheet.html)
  Java に限らず、JWT を利用する際のセキュリティベストプラクティスがまとまっています。
