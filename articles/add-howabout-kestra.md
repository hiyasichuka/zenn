...
これらは **「Airflow の宿命」** と感じていたのですが、**Kestra**というオープンソースのワークフローツールに触れてみると、 「この課題、解消できそう」と思いました。
...
スケジュール（毎日２時など）やイベント（ファイル到着、Webhook、別フローの完了など）をきっかけにした実行を、同じ書き方で簡単に作れます。  
Airflow でも同様のことは可能ですが、`Sensor` や `TriggerDagRunOperator` などを組み合わせる必要があり、構成が複雑になりがちです。  
...
| -------------- | -------------------------------------- | ---------------------------------------------------- |
| 記述言語       | Python                                 | YAML                                                 |
| 依存関係の指定 | 明示的に`>>`や`<<`で記述               | 単純な直列処理は順番に書くだけで依存関係が設定される |
...
- **Kestra の場合**
  - 「HTTP リクエスト」「ファイルアップロード」「外部サービス連携」など、多くの処理が**タスクタイプ**としてあらかじめ用意されています。
